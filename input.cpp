...
void input::get_input()
{
	...
	//Triggering events upon button pressed 
	SDL_Event event;
	while (SDL_PollEvent(&event))
	{
		...
		case SDL_QUIT:
			...
			//Here we are specifying the menu buttons used insIde the game using their ID
		case SDL_SYSWMEVENT:
			...
			else if (event.syswm.msg->msg.win.wParam == ID_CREDITS_DEVELOPER)
			{
				_button_state[Button::SHOW_CREDITS] = Button_State::PRESSED;
			}
			else if (event.syswm.msg->msg.win.wParam == ID_TOGGLE_ENABLETEXT)
			{
				_button_state[Button::DEBUG_SHOW_IDS] = Button_State::PRESSED;
			}

			//Here we are using the Buttons to control the player actions
		case SDL_KEYDOWN:
			switch (event.key.keysym.scancode)
			{
			case SDL_SCANCODE_F1:
				if (!is_button_state(Button::STARTBUTTON, Button_State::DOWN))
				{

					_button_state[Button::STARTBUTTON] = Button_State::PRESSED;
				}
				break;
			case SDL_SCANCODE_W:
		...
		case SDL_KEYUP:
			...
			{
			case SDL_SCANCODE_F1:
				_button_state[Button::STARTBUTTON] = Button_State::RELEASED;
				break;
			case SDL_SCANCODE_W:
...
//Checking the Input Button State
